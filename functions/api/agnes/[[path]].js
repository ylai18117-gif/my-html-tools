...
export async function onRequestGet() {
  return json({ ok: true, service: 'agnes-proxy' });
}

export async function onRequestPost(context) {
  const { request, params, env } = context;

  const apiKey = env.AGNES_API_KEY || request.headers.get('x-client-api-key') || '';
  if (!apiKey) {
  ...
  if (!parts.length) {
    return json({ error: 'Missing Agnes path' }, 400);
  }

  const upstreamUrl = `https://apihub.agnes-ai.com/${parts.join('/')}`;
